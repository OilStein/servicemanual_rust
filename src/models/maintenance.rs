...
impl MaintenanceBMC {
  ...
  pub async fn create<T: Creatable>(db: Data<SurrealDB>, tb: &str, data: T) -> Result<Object, Error> {
    ...
    let value = res.into_iter().next().map(|r| r.result).expect("failded to return id")?;
    W(value.first()).try_into()
  }


  pub async fn update<T: Patchable>(db: Data<SurrealDB>, mid: &str, data: T) -> Result<Object, Error> {
