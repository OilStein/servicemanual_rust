...
use crate::{database::surreal_db::SurrealDB, models::device::{Device, DeviceBMC}};



pub async fn populate(db: Data<SurrealDB>) -> Result<(), Box<dyn std::error::Error>> {
  let mut reader = csv::ReaderBuilder::new().has_headers(true).from_path("D:/rust/servicemanual_rust/data/devices.csv")?;
